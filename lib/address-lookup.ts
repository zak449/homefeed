/**
 * Address Lookup — searches for ANY property by address
 * Uses the Realty in US API to find properties that may not be actively listed
 * Shared between API route and server components (no self-fetch needed)
 */
...
export type AddressLookupResponse = {
  ...
  source: "db" | "api" | "api_error" | "not_found" | "none" | "error";
};
...
export async function lookupAddress(query: string): Promise<AddressLookupResponse> {
  ...
  if (dbResults.length > 0) {
    return { results: dbResults, source: "db" };
  }
  ...
  try {
    ...
    if (!autoRes.ok) {
      ...
      return { results: [], source: "api_error" };
    }
    ...
    if (!home) {
      return { results: [], source: "not_found" };
    }
...
async function searchByLocation(query: string): Promise<AddressLookupResponse> {
  try {
    ...
    if (!res.ok) return { results: [], source: "api_error" };
    ...
    // Upsert all results
    const upserted: AddressLookupResult[] = [];
    ...
    return { results: upserted, source: "api" };
  } catch (e) {
