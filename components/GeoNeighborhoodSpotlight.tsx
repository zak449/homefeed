    ...
    <div className="px-4 py-5">
      <div className="rounded-2xl bg-gradient-to-br from-amber/5 via-surface to-highlight border border-amber/15 p-5 relative overflow-hidden">
        ...
        <div className="relative">
          <p className="text-xs font-bold tracking-widest uppercase text-amber mb-2">Neighborhood Spotlight</p>
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="flex -space-x-2">
              {["ZK", "ML", "JR", "AS"].map((init) => (
                <div key={init} className="w-7 h-7 rounded-full bg-ink text-bg text-xs font-bold flex items-center justify-center border-2 border-surface">{init}</div>
              ))}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <a href={cityHref} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full border border-amber/30 text-amber text-sm font-semibold hover:bg-amber/10 active:scale-[0.97] transition-all">
              Browse {hasLocation ? displayCity : "Echo Park"} listings
            </a>
          </div>
        </div>
      </div>
